...
function updateInfo () 
	{
		...
		if(bestDifference>databaseLeaderFitness) 
			{
				//get database leader fitness for this image
				var client = new $.RestClient('/rest/api/');
				client.add('foo');
				var databaseLeaderFitness = client.foo.read(imageId.bestDifference);
				if(bestDifference>databaseLeaderFitness)
					{
						//overwrite best difference %, dna and svg on server
						//PUT	/users/:id	Replaces an existing document
						client.foo.put(imageId.bestDifference);
						client.foo.put(imageId.bestDNA);
						client.foo.put(imageId.bestCtx);
					}
			}
